...
class HermesCLI:
    ...
    def _handle_case_command(self, cmd: str):
        """Handle the /case command — delegate to the shared Work Case CLI."""
        from hermes_cli.case import run_slash

        rest = cmd.strip()
        if rest.startswith("/"):
            rest = rest.lstrip("/")
        if rest.startswith("case"):
            rest = rest[len("case"):].lstrip()
        try:
            output = run_slash(rest)
        except Exception as exc:  # pragma: no cover - defensive
            output = f"(._.) case error: {exc}"
        if output:
            print(output)
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            self._handle_case_command(cmd_original)
